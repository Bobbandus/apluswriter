...
- Konton: konto-knapp på alla sidor, **nyregistrering stängd** i Supabase, inbjudan manuellt. Inloggning **2026-09-22** bytt till lösenord som förstahandsval (Supabases mejlutskick är hårt begränsat utan egen SMTP) — magic link kvar som reserv för en inbjudens första inloggning och "Glömt lösenordet?" (`components/auth/LoginForm.tsx`, ny sida `components/auth/ResetPassword.tsx` på `/auth/reset-password`, lösenordsbyte i Inställningar → Konto). Kräver att `/auth/reset-password` läggs till bredvid `/auth/callback` under Redirect URLs i Supabase, se `supabase/README.md`.
- **2026-09-22, samma dag:** förnamn/efternamn på profilen, massflytt av lokala projekt till kontot vid inloggning (bannern på instrumentpanelen, `moveAllToCloud`), och delning — bjud in en person med en roll eller skapa en anonym läslänk (`components/projects/ShareSheet.tsx`, `lib/sharing/`, läsvyn på `/delad/[token]`). Ny SQL: `supabase/06_collaboration.sql` — **måste köras i SQL Editor**, den ligger bara i repot tills dess. Kommentering via en anonym länk är inte byggt; permissionen finns i gränssnittet men gör inget än.
...
- Handboll (klart): `packages/live/handball.ts` (klocka som räknar upp, halvlekar, utvisningar på 2 min som går ut med klockan, tre åt gången); ritas med samma designs som poängtavlan plus utvisningsbrickor. Tiden bärs av åtgärderna (`at`), inte av reducern. Två enheter är överens om klockan så långt deras egna klockor är det (någon sekund med nätverkstid); ingen serverklocka än.
- Stream Deck (klart): `GET|POST /api/live/<kontrollänk>/<knapp>` (`app/api/live/[token]/[name]/route.ts`, namnen i `packages/live/api.ts`, listade per tavla på /live). Samma regler och versionskontroll som kontrollsidan, skickar en nudge till utdatan. Bara Supabase-tavlor (en lokal tavla syns inte för servern). Ingen hastighetsbegränsning än; kontrollänken är lösenordet. Verifierat: felfall (ogiltig länk 404, saknad SQL 503); riktig knapptryckning kräver att 05_live.sql är körd.
