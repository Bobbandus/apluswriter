...
**Fas 5 — M9 Import/export:** [~] FDX ut klar (`packages/export/fdx.ts`, betoning bold/kursiv följer inte med än) · [x] HTML ut (`html.ts`) · [x] rapporter CSV (`reports.ts`: scener, roller, platser) · [ ] **▶ NÄSTA:** rollsidor som PDF · [x] FDX in (`packages/fountain/importFdx.ts`, via Importera fil; ingen förhandsgranskning eftersom det alltid blir ett nytt projekt; rundtur mot exporten testad på Big Fish) · [ ] Highland/Word in · [ ] titelsidesformulär
